...
api_key = 'f6bd5fc2a27b6aedccc022a5529f7a66',
...
aqiList = ["Good", "Fair", "Moderate", "Poor", "Deadly"];
...
function getWeatherDetails(name, lat, lon, country, state){
    ...
    fetch(WEATHER_API_URL).then(res => res.json()).then(data =>{
        ...
        sunriseCard.innerHTML = `
            ...
            pressureVal.innerHTML = `${pressure}pa`;
...
recentSearchesContainer.className = "recent-searches-box";
recentSearchesContainer.style.display = "none"; 
recentSearchesContainer.innerHTML = "<ul id='recent-searches'></ul>";
document.querySelector(".weather-input").appendChild(recentSearchesContainer);
...
function saveToRecentSearches(city) {
    ...
    if (!searches.includes(city)) {
        ...
        if (searches.length > 5) searches.pop(); // Limit to 5 recent searches
...
cityInput.addEventListener("click", displayRecentSearches);


document.addEventListener("click", (e) => {
    if (!cityInput.contains(e.target) && !recentSearchesContainer.contains(e.target)) {
        hideRecentSearches();
    }
});
...
searchBtn.addEventListener("click", () => {
    ...
    if (city) saveToRecentSearches(city);
});
